...
def app():
    ...
    * **ATTENTION:**
    ...
    if st.button('inference'):
        ...
        if df_true is None:
            ...
            def inc_df(df, current_t1, current_t2, shift_by):
                ...
                shift_index = _df[(_df.iloc[:, 0] == current_t1) & (
                    _df.iloc[:, 1] == current_t2)].index + shift_by
                shifted_t1 = _df.iloc[shift_index.to_list()[0], 0]
                shifted_t2 = _df.iloc[shift_index.to_list()[0], 1]
                return shifted_t1, shifted_t2

            # fromは現状使ってない。5-8週モデルで確認
            def inference_time_convert_df(df, df_true, _from, _to):
                time_conv = df.iloc[:, [1, 2]].drop_duplicates().sort_values(
                    [df.columns[1], df.columns[2]]).reset_index(drop=True)
                time_conv['temp_t1'] = None
                time_conv['temp_t2'] = None
                for i in time_conv.index:
                    shifted_t1, shifted_t2 = inc_df(
                        df_true, time_conv.iloc[i, 0], time_conv.iloc[i, 1], to_period)
            ...
            conv_df = inference_time_convert_df(
                X_inference, df_true, from_period, to_period)
            conv_df = pd.merge(X_inference.iloc[:, 0:3], conv_df)
            conv_df.drop(conv_df.columns[[1, 2]], axis=1, inplace=True)
            conv_df.rename(
                columns={
                    'temp_t1': X_inference.columns[1],
                    'temp_t2': X_inference.columns[2]},
                inplace=True)
            conv_df[X_inference.columns[0]] = conv_df.apply(id_pred, axis=1)
            # st.write(conv_df)
            df_inf = pd.concat([conv_df,
                                pd.DataFrame(df_inf,
                                             columns=[df_true.columns[3]]),
                                X_inference[[selected_classification_col]]],
                               axis=1)
    ...
    if st.button('Graph'):
        ...
        input_df = pd.merge(df_true,
                            input_df[[input_df.columns[0],
                                      selected_classification_col]].drop_duplicates())
        ...
        for c in sorted(selected_clusters):
            pivot_df = pivot_df_for_dengram(
                input_df[input_df[selected_classification_col] == c].iloc[:, 0:4])
            # st.write(pivot_df.T)
        ...
        for c in sorted(
                set(input_df['cluster'].drop_duplicates()) - set(selected_clusters)):
            pivot_df = pivot_df_for_dengram(
                input_df[input_df[selected_classification_col] == c].iloc[:, 0:4])
            # st.write(pivot_df.T)
            if not math.isnan(c):
